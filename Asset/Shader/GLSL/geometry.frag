...
void main() {
    ...
    Metallic = material.metallic_map < 1024?
        vec4(vec3(texture(textureArray[nonuniformEXT(material.metallic_map)], inUV).x * material.metallic_factor), 1.0) : 
        vec4(vec3(material.metallic_factor), 1.0);

    // Roughness G-Buffer
    Roughness = material.roughness_map < 1024?
        vec4(vec3(texture(textureArray[nonuniformEXT(material.roughness_map)], inUV).x * material.roughness_factor), 1.0) : 
