...
using SkinnedVertex = Resource<ResourceType::Model>::SkinnedVertex;
...
class AssimpImporter : public Importer<ResourceType::Model>
{
  public:
	...
	void ProcessAnimation(const aiScene *assimp_scene, ModelInfo &data)
	{
		for (uint32_t i = 0; i < assimp_scene->mNumAnimations; i++)
		{
			...
			for (uint32_t j = 0; j < assimp_animation->mNumChannels; j++)
			{
				...
				bones.emplace_back(bone_name, bone_id, std::move(key_positions), std::move(key_rotations), std::move(key_scales));
	...
	size_t ProcessMesh(const aiScene *scene, aiNode *node, ModelInfo &data)
	{
	...
	Node ProcessNode(const aiScene *assimp_scene, aiNode *assimp_node, ModelInfo &data, aiMatrix4x4 transform = aiMatrix4x4())
	{
		Node node = {};
		...
		node.mesh = ProcessMesh(assimp_scene, assimp_node, data);

		for (uint32_t i = 0; i < assimp_node->mNumChildren; i++)
		{
			Node child = ProcessNode(assimp_scene, assimp_node->mChildren[i], data, transform);
  ...
  protected:
	virtual std::unique_ptr<Resource<ResourceType::Model>> Import_(const std::string &path, RHIContext *rhi_context) override
	{
		...
		if (const aiScene *scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_CalcTangentSpace))
		{
			...
			data.root = ProcessNode(scene, scene->mRootNode, data, identity);
