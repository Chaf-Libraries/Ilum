...
class DiffuseTransmissionBSDF : public MaterialNode<DiffuseTransmissionBSDF>
{
  public:
	virtual MaterialNodeDesc Create(size_t &handle) override
	{
		...
		return desc
		    .SetHandle(handle++)
		    .SetName("DiffuseTransmissionBSDF")
		    .SetCategory("BSDF")
...
CONFIGURATION_MATERIAL_NODE(DiffuseTransmissionBSDF)