...
float gHistoryFixStrength : packoffset(c26);
}

#line 1 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_HistoryFix.cs.usf"
#line 11 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_HistoryFix.cs.usf"
...
#line 12 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_HistoryFix.cs.usf"
#line 13 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_HistoryFix.cs.usf"
#line 1 "STL.ush"
...
#line 14 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_HistoryFix.cs.usf"
#line 1 "NRD.ush"
...
#line 15 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_HistoryFix.cs.usf"
#line 19 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_HistoryFix.cs.usf"
...
#line 20 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_HistoryFix.cs.usf"
#line 1 "REBLUR_DiffuseSpecular_HistoryFix.resources.ush"
#line 11 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.resources.ush"
...
#line 59 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.resources.ush"

        Texture2D<float4> gIn_Normal_Roughness;
        Texture2D<float4> gIn_Data1;
        Texture2D< float2 > gIn_Diff;
#line 70 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.resources.ush"



        RWTexture2D< float2 > gOut_Diff;
#line 77 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.resources.ush"

#line 21 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_HistoryFix.cs.usf"
#line 22 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_HistoryFix.cs.usf"
#line 1 "Common.ush"
...
#line 23 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_HistoryFix.cs.usf"
...
#line 24 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_HistoryFix.cs.usf"
#line 1 "REBLUR_DiffuseSpecular_HistoryFix.ush"
#line 11 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.ush"
groupshared float2 s_FrameNum[  ( 16 + 2 * 2 )  ][  ( 16 + 2 * 2 )  ];


groupshared float s_DiffLuma[  ( 16 + 2 * 2 )  ][  ( 16 + 2 * 2 )  ];
groupshared float s_SpecLuma[  ( 16 + 2 * 2 )  ][  ( 16 + 2 * 2 )  ];
...
#line 66 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.ush"
        viewZ =  ( viewZ / 0.125 ) ;
#line 86 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.ush"
    int2 groupBase = pixelPos - threadPos - 2 ; uint stageNum = ( ( 16 + 2 * 2 ) * ( 16 + 2 * 2 ) + 16 * 16 - 1 ) / ( 16 * 16 ); [unroll] for( uint stage = 0; stage < stageNum; stage++ ) { uint virtualIndex = threadIndex + stage * 16 * 16 ; uint2 newId = uint2( virtualIndex % ( 16 + 2 * 2 ) , virtualIndex / ( 16 + 2 * 2 ) ); if( stage == 0 || virtualIndex < ( 16 + 2 * 2 ) * ( 16 + 2 * 2 ) ) Preload( newId, groupBase + newId ); } GroupMemoryBarrierWithGroupSync( ) ;
    ...
    float2 frameNumUnclamped = s_FrameNum[ smemPos.y ][ smemPos.x ];
            ...
            float diffMax = - 1e6 ;
            float diffMin =  1e6 ;
#line 111 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.ush"
        [unroll]
        for( int y = 0; y <=  2  * 2; y++ )
        {
            [unroll]
...
#line 147 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.ush"
            }
        }

        float invSum = 1.0 / ( (  2  * 2 + 1 ) * (  2  * 2 + 1 ) );
        float useFastHistory = float( gMaxAccumulatedFrameNum > gMaxFastAccumulatedFrameNum );


            float diffLuma = GetLuma( diff );
#line 167 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.ush"
...
#line 210 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.ush"
    float2 frameNum = saturate( frameNumUnclamped /  3.0  );
    float2 c = frameNum;
    float2 sum = 1.0;

    [unroll]
    for( int dy = 0; dy <=  2  * 2; dy++ )
    {
        [unroll]
        for( int dx = 0; dx <=  2  * 2; dx++ )
        {
            ...
            float2 o = float2( dx, dy ) -  2 ;
            if( any( abs( o ) > 1 ) )
                continue;

            int2 pos = threadPos + int2( dx, dy );

            float2 s = saturate( s_FrameNum[ pos.y ][ pos.x ] /  3.0  );
            float2 w = step( c, s );
    ...
    float2 scale = saturate( 1.0 - frameNum );
        ...
        stepSize = scale.x;
#line 252 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.ush"
    sum = 1.0;
    ...
    if( stepSize >  0.111  )
    {
        ...
        stepSize *=  ( 10.0 * ( gRectSize.y / 1440.0 ) ) ;
        stepSize *= gHistoryFixStrength;


            float diffNonLinearAccumSpeed = 1.0 / ( 1.0 + frameNumUnclamped.x );
            float2 diffGeometryWeightParams = GetGeometryWeightParams( gPlaneDistSensitivity, frustumHeight, Xv, Nv, lerp( 1.0,  0.05 , diffNonLinearAccumSpeed ) );
            float diffNormalWeightParam = GetNormalWeightParams( diffNonLinearAccumSpeed, 1.0 );
#line 306 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.ush"
            diff *= sum.x;
#line 319 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.ush"
        [unroll]
        for( int i = -2; i <= 2; i++ )
        {
            [unroll]
            for( int j = -2; j <= 2; j++ )
            {
                ...
                float2 uvScaled = uv * gResolutionScale;
                    ...
                    float z;
                        ...
                        float d = diffTemp.x;
                        z = diffTemp.y;
#line 350 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.ush"
                    z =  ( z / 0.125 ) ;
#line 361 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.ush"
                float materialIDs;
...
#line 380 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.ush"
                ...
                float angle = STL::Math::AcosApprox( cosa );
                    ...
                    w.x *=  rcp( ( - 3.0 * abs( ( angle ) * ( diffNormalWeightParam ) + ( 0.0 ) ) ) * ( - 3.0 * abs( ( angle ) * ( diffNormalWeightParam ) + ( 0.0 ) ) ) - ( - 3.0 * abs( ( angle ) * ( diffNormalWeightParam ) + ( 0.0 ) ) ) + 1.0 ) ;
#line 395 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.ush"
                    diff += d * w.x;
#line 409 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.ush"
                sum += w;
            }
        }
    }
    ...
    sum = rcp( sum );
            ...
            gOut_Diff[ pixelPos ] = float2( diff * sum.x,  min( viewZ * 0.125 , 65504.0 )  );
#line 438 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_HistoryFix.ush"
}
#line 25 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_HistoryFix.cs.usf"
