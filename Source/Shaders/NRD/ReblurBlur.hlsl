...
{
float4 gRotator : packoffset(c0);
float3 gSpecLobeTrimmingParams : packoffset(c1);
float gBlurRadiusScale : packoffset(c1.w);
float4x4 gViewToClip : packoffset(c2);
float4x4 gViewToWorld : packoffset(c6);
float4 gFrustum : packoffset(c10);
float4 gHitDistParams : packoffset(c11);
float4 gViewVectorWorld : packoffset(c12);
float4 gViewVectorWorldPrev : packoffset(c13);
float2 gInvScreenSize : packoffset(c14);
float2 gScreenSize : packoffset(c14.z);
float2 gInvRectSize : packoffset(c15);
float2 gRectSize : packoffset(c15.z);
float2 gRectSizePrev : packoffset(c16);
float2 gResolutionScale : packoffset(c16.z);
float2 gRectOffset : packoffset(c17);
float2 gSensitivityToDarkness : packoffset(c17.z);
uint2 gRectOrigin : packoffset(c18);
float gReference : packoffset(c18.z);
float gOrthoMode : packoffset(c18.w);
float gUnproject : packoffset(c19);
float gDebug : packoffset(c19.y);
float gDenoisingRange : packoffset(c19.z);
float gPlaneDistSensitivity : packoffset(c19.w);
float gFramerateScale : packoffset(c20);
float gBlurRadius : packoffset(c20.y);
float gMaxAccumulatedFrameNum : packoffset(c20.z);
float gAntiFirefly : packoffset(c20.w);
float gMinConvergedStateBaseRadiusScale : packoffset(c21.y);
float gLobeAngleFraction : packoffset(c21.z);
float gRoughnessFraction : packoffset(c21.w);
float gResponsiveAccumulationRoughnessThreshold : packoffset(c22);
float gDiffPrepassBlurRadius : packoffset(c22.y);
float gSpecPrepassBlurRadius : packoffset(c22.z);
uint gIsWorldSpaceMotionEnabled : packoffset(c22.w);
uint gFrameIndex : packoffset(c23);
uint gResetHistory : packoffset(c23.y);
uint gDiffMaterialMask : packoffset(c23.z);
uint gSpecMaterialMask : packoffset(c23.w);
}

#line 1 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_Blur.cs.usf"
#line 11 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_Blur.cs.usf"
...
#line 12 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_Blur.cs.usf"
#line 13 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_Blur.cs.usf"
#line 1 "STL.ush"
...
#line 14 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_Blur.cs.usf"
#line 1 "NRD.ush"
...
#line 15 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_Blur.cs.usf"
#line 19 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_Blur.cs.usf"
...
#line 20 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_Blur.cs.usf"
#line 1 "REBLUR_DiffuseSpecular_Blur.resources.ush"
#line 11 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_Blur.resources.ush"
...
#line 59 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_Blur.resources.ush"

        Texture2D<float4> gIn_Normal_Roughness;
        Texture2D<float4> gIn_Data1;
        Texture2D< float2 > gIn_Diff;
#line 69 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_Blur.resources.ush"



        RWTexture2D< float2 > gOut_Diff;
        RWTexture2D<float> gOut_ViewZ;
#line 77 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_Blur.resources.ush"

#line 21 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_Blur.cs.usf"
#line 22 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_Blur.cs.usf"
#line 1 "Common.ush"
...
#line 23 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_Blur.cs.usf"
...
#line 24 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_Blur.cs.usf"
#line 1 "REBLUR_DiffuseSpecular_Blur.ush"
#line 11 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_Blur.ush"
[numthreads(  8 ,  8 , 1 )]
 void  MainCS ( int2 threadPos : SV_GroupThreadId, int2 pixelPos : SV_DispatchThreadId, uint threadIndex : SV_GroupIndex )
{
    ...
    float2 pixelUv = float2( pixelPos + 0.5 ) * gInvRectSize;
        ...
        float viewZ;
            ...
            float2 diffTemp = gIn_Diff.SampleLevel(gLinearClamp, pixelUv, 0);
            viewZ = diffTemp.y;
#line 31 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_Blur.ush"
        viewZ =  ( viewZ / 0.125 ) ;
#line 37 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_Blur.ush"
    gOut_ViewZ[ pixelPos ] =  min( viewZ * 0.125 , 65504.0 ) ;
    ...
    float materialID;
    float4 normalAndRoughness = NRD_FrontEnd_UnpackNormalAndRoughness( gIn_Normal_Roughness.SampleLevel(gLinearClamp, pixelUv, 0), materialID );
    float3 N = normalAndRoughness.xyz;
    float3 Nv = STL::Geometry::RotateVectorInverse( gViewToWorld, N );
    float roughness = normalAndRoughness.w;


    float4 internalData1 = UnpackInternalData1( gIn_Data1.SampleLevel(gLinearClamp, pixelUv, 0) );


    float3 Xv = STL::Geometry::ReconstructViewPosition( pixelUv, gFrustum, viewZ, gOrthoMode );
    float4 rotator = GetBlurKernelRotation(  0 , pixelPos, gRotator, gFrameIndex );
...
#line 71 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_Blur.ush"
#line 1 "REBLUR_Common_DiffuseSpatialFilter.ush"
#line 23 "/Plugin/NRD/Private/Reblur/REBLUR_Common_DiffuseSpatialFilter.ush"
{
        ...
        float2 sum = 1.0;
        float boost = saturate( 1.0 - diffInternalData.y /  3.0  );
        ...
        float lobeAngleFractionScale = gLobeAngleFraction *  2.0 ;
#line 54 "/Plugin/NRD/Private/Reblur/REBLUR_Common_DiffuseSpatialFilter.ush"
        ...
        float2x3 TvBv = GetKernelBasis( Nv, Nv, 1.0, worldBlurRadius );
        float2 geometryWeightParams = GetGeometryWeightParams( gPlaneDistSensitivity, frustumHeight, Xv, Nv, lerp( 1.0,  0.05 , diffInternalData.x ) );
...
#line 92 "/Plugin/NRD/Private/Reblur/REBLUR_Common_DiffuseSpatialFilter.ush"
        float2 minHitDistWeight = float2( 0.0,  0.1  );
#line 98 "/Plugin/NRD/Private/Reblur/REBLUR_Common_DiffuseSpatialFilter.ush"
        diff *= sum.xxxy;
#line 103 "/Plugin/NRD/Private/Reblur/REBLUR_Common_DiffuseSpatialFilter.ush"
        [unroll]
        for( uint i = 0; i <  8 ; i++ )
        {
...
#line 123 "/Plugin/NRD/Private/Reblur/REBLUR_Common_DiffuseSpatialFilter.ush"
            float2 uvScaled = uv * gResolutionScale;
                ...
                float zs = data.y;
                float4 s = data.x;
#line 146 "/Plugin/NRD/Private/Reblur/REBLUR_Common_DiffuseSpatialFilter.ush"
            zs =  ( zs / 0.125 ) ;
            ...
            w *=  ( ( gDiffMaterialMask ) == 0 ? 1.0 : ( ( materialID ) == ( materialIDs ) ) ) ;
            w *= GetGaussianWeight( offset.z );
            w *= GetCombinedWeight( geometryWeightParams, Nv, Xvs, normalWeightParams, N, Ns );
#line 172 "/Plugin/NRD/Private/Reblur/REBLUR_Common_DiffuseSpatialFilter.ush"
            float2 ww = w;
            ...
            sum += ww;
            diff += s * ww.xxxy;
#line 186 "/Plugin/NRD/Private/Reblur/REBLUR_Common_DiffuseSpatialFilter.ush"
        }

        float2 invSum = STL::Math::PositiveRcp( sum );
        diff *= invSum.xxxy;
#line 220 "/Plugin/NRD/Private/Reblur/REBLUR_Common_DiffuseSpatialFilter.ush"
        gOut_Diff[ pixelPos ] = float2( diff.w,  min( viewZ * 0.125 , 65504.0 )  );
#line 230 "/Plugin/NRD/Private/Reblur/REBLUR_Common_DiffuseSpatialFilter.ush"
}
#line 72 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_Blur.ush"
#line 87 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseSpecular_Blur.ush"
}
#line 25 "/Plugin/NRD/Private/Reblur/REBLUR_DiffuseOcclusion_Blur.cs.usf"
