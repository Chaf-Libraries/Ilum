Texture2D BloomUpSampleLow : register(t0);
SamplerState BloomUpSampleSampler : register(s0);
Texture2D BloomUpSampleHigh : register(t1);
RWTexture2D<float4> BloomUpSampleOut : register(u2);

[[vk::push_constant]]
struct
{
    float radius;
} upsample_push_constants;
...
void main(CSParam param)
{
    ...
    uint2 extent;
    BloomUpSampleHigh.GetDimensions(extent.x, extent.y);
    ...
    Store2Pixels(
        start + 0,
        lerp(
            BloomUpSampleHigh.SampleLevel(BloomUpSampleSampler, (float2(ThreadUL + uint2(0, 0)) + float2(0.5, 0.5)) * pixel_size, 0.0).rgb,
            BloomUpSampleLow.SampleLevel(BloomUpSampleSampler, (float2(ThreadUL + uint2(0, 0)) + float2(0.5, 0.5)) * pixel_size, 0.0).rgb,
            upsample_push_constants.radius
        ),
        lerp(
            BloomUpSampleHigh.SampleLevel(BloomUpSampleSampler, (float2(ThreadUL + uint2(1, 0)) + float2(0.5, 0.5)) * pixel_size, 0.0).rgb,
            BloomUpSampleLow.SampleLevel(BloomUpSampleSampler, (float2(ThreadUL + uint2(1, 0)) + float2(0.5, 0.5)) * pixel_size, 0.0).rgb,
            upsample_push_constants.radius
        )
    );
    ...
    Store2Pixels(
        start + 8,
        lerp(
            BloomUpSampleHigh.SampleLevel(BloomUpSampleSampler, (float2(ThreadUL + uint2(0, 1)) + float2(0.5, 0.5)) * pixel_size, 0.0).rgb,
            BloomUpSampleLow.SampleLevel(BloomUpSampleSampler, (float2(ThreadUL + uint2(0, 1)) + float2(0.5, 0.5)) * pixel_size, 0.0).rgb,
            upsample_push_constants.radius
        ),
        lerp(
            BloomUpSampleHigh.SampleLevel(BloomUpSampleSampler, (float2(ThreadUL + uint2(1, 1)) + float2(0.5, 0.5)) * pixel_size, 0.0).rgb,
            BloomUpSampleLow.SampleLevel(BloomUpSampleSampler, (float2(ThreadUL + uint2(1, 1)) + float2(0.5, 0.5)) * pixel_size, 0.0).rgb,
            upsample_push_constants.radius
        )
    );
    ...
    BloomUpSampleOut[param.DispatchThreadID.xy] = float4(BlurVertically((param.GroupThreadID.y << 3) + param.GroupThreadID.x), 1.0);
}