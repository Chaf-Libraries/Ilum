...
float GetLuma(float2 uv, float2 offset, float2 texel_size)
{
    ...
    return InImage.SampleLevel(TexSampler, uv, 0.0).a;
}
...
float GetSubpixelBlendFactor(LumaNeighborhood luma)
{
    ...
    return filter * filter * push_constants.subpixel_blending;
}

bool FXAASkip(LumaNeighborhood luma)
{
    return luma.range < max(push_constants.fixed_threshold, push_constants.relative_threshold * luma.highest);
}
...
float GetEdgeBlendFactor(LumaNeighborhood luma, FXAAEdge edge, float2 uv, float2 texel_size)
{
    ...
    for (i = 0; i < EXTRA_EDGE_STEPS && !atN; i++)
    {
...
void main(CSParam param)
{
    ...
    if (FXAASkip(luma))
    {
        OutImage[param.DispatchThreadID.xy] = float4(InImage[param.DispatchThreadID.xy].rgb, 1.0);
        return;
    }
    ...
    OutImage[param.DispatchThreadID.xy] = float4(InImage.SampleLevel(TexSampler, uv, 0.0).rgb, 1.0);
    //OutImage[param.DispatchThreadID.xy] = float4(blend_factor, blend_factor, blend_factor, 1.0);
