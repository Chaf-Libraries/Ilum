...
struct PointLight
{
    ...
    bool IsIntersect(RayDesc ray, out float3 p)
    {
        ...
        float c = dot(ray.Origin - position, ray.Origin - position) - range * range;
    ...
    float3 Li(float3 frag_pos, out float3 wi)
    {
        ...
        float attenuation = max(min(1.0 - pow(d / range, 4), 1), 0) / (d * d);
    ...
    float3 SampleLi(Interaction interaction, float2 u, out float3 wi, out float pdf, out VisibilityTester visibility)
    {
        ...
        {
            float3 p = range * UniformSampleSphere(u) + position;
                        
            pdf = 1.0 / (2 * PI * range * range);
    ...
    float3 Le()
    {
        return color * intensity / (2 * PI * range * range);
    }
};
