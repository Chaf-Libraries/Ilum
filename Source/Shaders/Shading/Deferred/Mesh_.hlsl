...
struct MSOutput
{
    ...
    uint3 VBuffer : SV_Target4; // VBuffer: InstanceID, MeshletID, PrimitiveID
};
...
[numthreads(32, 1, 1)]void MSmain(CSParam param, in payload Payload pay_load, out vertices VertexOut verts[64], out indices uint3 tris[126], out primitives PrimitiveOut prims[126])
{
	...
	for (uint i = param.GroupThreadID.x; i < meshlet.vertex_count; i += 32)
	{
        ...
        verts[i].Normal = normalize(mul(vertex.normal.xyz, (float3x3) transform));
...
MSOutput PSmain(VertexOut input, PrimitiveOut prims)
{
	MSOutput output;
	...
	{
		output.GBuffer0.rgb = textureArray[NonUniformResourceIndex(materials[input.InstanceID].textures[TEXTURE_BASE_COLOR])].Sample(texSampler, input.TexCoord).rgb * materials[input.InstanceID].base_color.rgb;
	}
	else
	{
	...
	{
		output.GBuffer0.a = textureArray[NonUniformResourceIndex(materials[input.InstanceID].textures[TEXTURE_METALLIC])].Sample(texSampler, input.TexCoord).r * materials[input.InstanceID].metallic;
	}
	else
	{
	...
	{
		output.GBuffer2.rgb = textureArray[NonUniformResourceIndex(materials[input.InstanceID].textures[TEXTURE_EMISSIVE])].Sample(texSampler, input.TexCoord).rgb * materials[input.InstanceID].emissive_color * materials[input.InstanceID].emissive_intensity;
	}
	else
	{
	...
	{
		output.GBuffer2.a = textureArray[NonUniformResourceIndex(materials[input.InstanceID].textures[TEXTURE_ROUGHNESS])].Sample(texSampler, input.TexCoord).g * materials[input.InstanceID].roughness;
	}
	else
	{
    ...
    output.VBuffer.x = input.InstanceID;
    output.VBuffer.y = prims.PrimitiveID;
    output.VBuffer.z = prims.MeshletID;
