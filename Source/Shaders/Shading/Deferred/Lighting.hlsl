...
[numthreads(LOCAL_SIZE, LOCAL_SIZE, 1)] void main(CSParam param) {
	uint2 extent;
	...
	float4 gbuffer0 = GBuffer0.SampleLevel(TextureSampler, uv, 0.0);        // GBuffer0: RGB - Albedo, A - metallic
	float4 gbuffer1 = GBuffer1.SampleLevel(TextureSampler, uv, 0.0);        // GBuffer1: RGB - normal, A - linear depth
	float4 gbuffer2 = GBuffer2.SampleLevel(TextureSampler, uv, 0.0);        // GBuffer2: RGB - emissive, A - roughness
	float4 gbuffer3 = GBuffer3.SampleLevel(TextureSampler, uv, 0.0);        // GBuffer3: R - entity id, G - instance id, BA - motion vector
	float  depth    = DepthBuffer.SampleLevel(TextureSampler, uv, 0.0).r;
