...
Image::Image(const Device &device, uint32_t width, uint32_t height, VkFormat format, VkImageUsageFlags usage, VmaMemoryUsage memory_usage, bool mip_level, uint32_t layer_count) :
    m_device(device), m_extent({width, height}), m_format(format)
{
	...
	vmaCreateImage(m_device.GetAllocator(), &image_create_info, &allocation_create_info, &m_handle, &m_allocation, nullptr);

	CreateImageViews();
}

Image::Image(const Device &device, VkImage image, uint32_t width, uint32_t height, VkFormat format) :
    m_device(device), m_handle(image), m_extent({width, height}), m_format(format)
{
	CreateImageViews();
}
...
Image &Image::operator=(Image &&other) noexcept
{
	...
	assert(m_device == other.m_device);
...
const VkImageView &Image::GetView(ImageViewType type) const
{
	switch (type)
	{
		case ImageViewType::Native:
			return m_views.native;
		case ImageViewType::Depth_Only:
			return m_views.depth;
		case ImageViewType::Stencil_Only:
...
VkImageSubresourceLayers Image::GetSubresourceLayers(uint32_t mip_level, uint32_t layer) const
{
	return VkImageSubresourceLayers{
	    /*aspectMask*/ FormatToAspect(m_format),
	    /*mipLevel*/ mip_level,
...
VkImageAspectFlags Image::FormatToAspect(VkFormat format)
{
	switch (format)
	{
...
VkImageLayout Image::UsageToLayout(VkImageUsageFlagBits usage)
{
	switch (usage)
	{
...
VkAccessFlags Image::UsageToAccess(VkImageUsageFlagBits usage)
{
	switch (usage)
	{
		case VK_IMAGE_USAGE_TRANSFER_SRC_BIT:
...
VkPipelineStageFlags Image::UsageToStage(VkImageUsageFlagBits usage)
{
	switch (usage)
	{
		case VK_IMAGE_USAGE_TRANSFER_SRC_BIT:
...
void Image::CreateImageViews()
{
	...
	vkCreateImageView(m_device, &view_create_info, nullptr, &m_views.native);
	...
	if (depth_range.aspectMask != 0)
	{
		...
		vkCreateImageView(m_device, &view_create_info, nullptr, &m_views.depth);
	}
	...
	if (stencil_range.aspectMask != 0)
	{
		...
		vkCreateImageView(m_device, &view_create_info, nullptr, &m_views.stencil);
	}
	...
	for (auto &views : m_layer_views)
	{
		...
		view_create_info.subresourceRange = native_range;
		vkCreateImageView(m_device, &view_create_info, nullptr, &views.native);
		...
		{
			view_create_info.subresourceRange = depth_range;
			vkCreateImageView(m_device, &view_create_info, nullptr, &views.depth);
		}
		...
		{
			view_create_info.subresourceRange = stencil_range;
			vkCreateImageView(m_device, &view_create_info, nullptr, &views.stencil);
		}
...
void Image::Destroy()
{
	if (m_handle)
	{
		...
		vkDestroyImageView(m_device, m_views.native, nullptr);
		if (m_views.depth)
		{
			vkDestroyImageView(m_device, m_views.depth, nullptr);
		}
		if (m_views.stencil)
		{
			vkDestroyImageView(m_device, m_views.stencil, nullptr);
		}

		for (auto &views : m_layer_views)
		{
			vkDestroyImageView(m_device, views.native, nullptr);
			if (views.depth)
			{
				vkDestroyImageView(m_device, views.depth, nullptr);
			}
			if (views.stencil)
			{
				vkDestroyImageView(m_device, views.stencil, nullptr);
			}
		}
	}
