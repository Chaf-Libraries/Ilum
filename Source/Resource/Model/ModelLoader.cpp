...
std::unique_ptr<Model> ModelLoader::Load(const std::string &file_path)
{
...
void ModelLoader::ParseNode(const std::string &file_path, aiMatrix4x4 &transform, const aiNode *node, const aiScene *scene, Model &model, std::vector<uint32_t> &meshlet_offsets, std::vector<uint32_t> &meshlet_counts)
{
	...
	for (uint32_t i = 0; i < node->mNumMeshes; i++)
	{
		...
		SubMesh submesh;
		ParseMaterial(file_path, material, submesh.material);
		submesh.index         = node->mMeshes[i];
		submesh.pre_transform = to_matrix(transform);
		submesh.bound.min     = to_vector(mesh->mAABB.mMin);
		submesh.bound.max     = to_vector(mesh->mAABB.mMax);
		model.bound.min       = glm::min(model.bound.min, submesh.bound.min);
		model.bound.max       = glm::min(model.bound.max, submesh.bound.max);
...
void ModelLoader::ParseMaterial(const std::string &file_path, const aiMaterial *mesh_material, Material &material)
{
	...
	{
		material.base_color_map = dictionary + path.C_Str();
	}
	path.Clear();
