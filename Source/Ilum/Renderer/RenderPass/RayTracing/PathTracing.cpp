...
{
	state.shader.load(std::string(PROJECT_SOURCE_DIR) + "Source/Shaders/RayTracing/PathTracing.rgen", VK_SHADER_STAGE_RAYGEN_BIT_KHR, Shader::Type::GLSL);
	state.shader.load(std::string(PROJECT_SOURCE_DIR) + "Source/Shaders/RayTracing/PathTracing.rchit", VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR, Shader::Type::GLSL);
	state.shader.load(std::string(PROJECT_SOURCE_DIR) + "Source/Shaders/RayTracing/PathTracing.rmiss", VK_SHADER_STAGE_MISS_BIT_KHR, Shader::Type::GLSL);
	...
	state.descriptor_bindings.bind(0, 1, "PathTracing", VK_DESCRIPTOR_TYPE_STORAGE_IMAGE);
...
void PathTracing::render(RenderPassState &state)
{
	...
	if (Renderer::instance()->Render_Stats.static_mesh_count.meshlet_count > 0 && vertex_buffer.getBuffer() && index_buffer.getBuffer())
	{
		...
		vkCmdPushConstants(cmd_buffer, state.pass.pipeline_layout, VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR, 0, sizeof(m_push_block), &m_push_block);
