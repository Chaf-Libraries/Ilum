...
namespace Ilum
{
RenderPassDesc ReblurBlur::CreateDesc()
{
	RenderPassDesc desc = {};
	desc.SetName("ReblurBlur")
	    ...
	    .Write("Out_ViewZ", RenderResourceDesc::Type::Texture, RHIResourceState::UnorderedAccess);

	return desc;
}

RenderGraph::RenderTask ReblurBlur::Create(const RenderPassDesc &desc, RenderGraphBuilder &builder, Renderer *renderer)
{
	auto *shader = renderer->RequireShader("Source/Shaders/NRD/ReblurBlur.hlsl", "MainCS", RHIShaderStage::Compute, {}, true);
	...
	struct
	{
		glm::vec4  gRotator                                  = {0.2651f, -0.9642f, 0.9642f, 0.2651f};
		glm::vec3  gSpecLobeTrimmingParams                   = {0.9500f, 0.0400f, 0.1100f};
		float      gBlurRadiusScale                          = 5;
		...
		glm::vec4  gFrustum                                  = glm::vec4(-1.0000f, 0.5625f, 2.0000f, -1.1250f);
		glm::vec4  gHitDistParams                            = glm::vec4(3.0000f, 0.1000f, 20.0000f, -25.0000f);
		glm::vec4  gViewVectorWorld                          = glm::vec4(-0.0234f, 0.9910f, 0.1315f, 0.0000f);
		glm::vec4  gViewVectorWorldPrev                      = glm::vec4(-0.0234f, 0.9910f, 0.1315f, 0.0000f);
	...
	return [=](RenderGraph &render_graph, RHICommand *cmd_buffer, rttr::variant &config) {
		...
		auto *out_view_z = render_graph.GetCUDATexture(desc.resources.at("Out_ViewZ").handle);
		...
		descriptor
		    ...
		    .BindTexture("gOut_ViewZ", out_view_z, RHITextureDimension::Texture2D);
