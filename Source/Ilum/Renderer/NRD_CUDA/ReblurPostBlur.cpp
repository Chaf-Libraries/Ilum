...
namespace Ilum
{
RenderPassDesc PostReblurBlur::CreateDesc()
{
	RenderPassDesc desc = {};
	desc.SetName("PostReblurBlur")
	    ...
	    .Write("Out_Normal_Roughness", RenderResourceDesc::Type::Texture, RHIResourceState::ShaderResource)
	    .Write("Out_Diff", RenderResourceDesc::Type::Texture, RHIResourceState::ShaderResource)
	    .Write("Out_AccumSpeeds_MaterialID", RenderResourceDesc::Type::Texture, RHIResourceState::ShaderResource);
	return desc;
}

RenderGraph::RenderTask PostReblurBlur::Create(const RenderPassDesc &desc, RenderGraphBuilder &builder, Renderer *renderer)
{
	auto *shader = renderer->RequireShader("Source/Shaders/NRD/ReblurPostBlur.hlsl", "MainCS", RHIShaderStage::Compute, {}, true);
	...
	struct
	{
		glm::vec4  gRotator                                  = {-0.9642f, -0.2651f, 0.2651f, -0.9642f};
		glm::vec3  gSpecLobeTrimmingParams                   = {0.9500f, 0.0400f, 0.1100f};
		float      gBlurRadiusScale                          = 5.f;
		...
		glm::vec2  gResolutionScale                          = {1.0000f, 1.0000f};
		glm::vec2  gRectOffset                               = {0.0000f, 0.0000f};
	...
	return [=](RenderGraph &render_graph, RHICommand *cmd_buffer, rttr::variant &config) {
		...
		auto *out_normal_roughness         = render_graph.GetCUDATexture(desc.resources.at("Out_Normal_Roughness").handle);
		auto *out_diff                     = render_graph.GetCUDATexture(desc.resources.at("Out_Diff").handle);
		auto *out_accum_speeds_material_id = render_graph.GetCUDATexture(desc.resources.at("Out_AccumSpeeds_MaterialID").handle);

		descriptor
		    ->BindBuffer("_RootShaderParameters", root_shader_parameter_buffer.get())
		    .BindSampler("gNearestMirror", renderer->GetRHIContext()->CreateSampler(SamplerDesc::NearestClamp))
		    ...
		    .BindTexture("gOut_Normal_Roughness", out_normal_roughness, RHITextureDimension::Texture2D)
		    .BindTexture("gOut_Diff", out_diff, RHITextureDimension::Texture2D)
		    .BindTexture("gOut_AccumSpeeds_MaterialID", out_accum_speeds_material_id, RHITextureDimension::Texture2D);
