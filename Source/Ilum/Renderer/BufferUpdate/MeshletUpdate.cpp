...
//				{
//					Renderer::instance()->Render_Stats.static_mesh_count.meshlet_count += submesh.meshlet_count;
//					Renderer::instance()->Render_Stats.static_mesh_count.triangle_count += submesh.indices_count / 3;
//				}
//			}
//		}
//
//		if (Renderer::instance()->Render_Stats.static_mesh_count.meshlet_count * sizeof(uint64_t) > Renderer::instance()->Render_Buffer.Meshlet_Buffer.getSize())
//		{
//			GraphicsContext::instance()->getQueueSystem().waitAll();
//			Renderer::instance()->Render_Buffer.Meshlet_Buffer = Buffer(Renderer::instance()->Render_Stats.static_mesh_count.meshlet_count * sizeof(uint64_t), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VMA_MEMORY_USAGE_GPU_ONLY);
//			Renderer::instance()->update();
//		}
//
//		Buffer meshlet_staging = Buffer(Renderer::instance()->Render_Stats.static_mesh_count.meshlet_count * sizeof(uint64_t), VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU);
//		uint64_t *meshlet_staging_data = reinterpret_cast<uint64_t*>(meshlet_staging.map());
//
//		// Update static mesh material
//		tbb::parallel_for(tbb::blocked_range<size_t>(0, static_mesh_view.size()), [&static_mesh_view, &meshlet_staging_data, meshlet_offset, instance_offset](const tbb::blocked_range<size_t> &r) {
