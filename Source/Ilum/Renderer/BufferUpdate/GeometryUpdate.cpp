...
void GeometryUpdate::run()
{
	    ...
	    resource_cache.getIndicesCount() * sizeof(uint32_t) != static_index_buffer.GetSize())
	{
		...
		static_index_buffer  = Graphics::Buffer(Graphics::RenderContext::GetDevice(), resource_cache.getIndicesCount() * sizeof(uint32_t), VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VMA_MEMORY_USAGE_GPU_ONLY);
