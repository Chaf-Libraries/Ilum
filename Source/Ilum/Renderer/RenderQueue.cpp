...
bool RenderQueue::update()
{
	...
	//	tbb::parallel_for_each(mesh_renderer.materials.begin(), mesh_renderer.materials.end(), [&instance_count](scope<Material> &material) {
	//		instance_count.fetch_add(1, std::memory_order_relaxed);
	//	});
	//});

	//if (/*instance_count != Renderer::instance()->Static_Instance_Count || */cmpt::Renderable::update)
	//{
	...
	//			auto *material = material_ptr->type() == typeid(material::PBRMaterial) ? static_cast<material::PBRMaterial *>(material_ptr.get()) : nullptr;
	...
	//	// Enlarge buffer
	//	//if (Instance_Visibility_Buffer.getSize() < instance_data.size() * sizeof(PerInstanceData))
	//	//{
