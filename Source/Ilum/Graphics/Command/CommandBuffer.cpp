    ...
    m_command_pool(Engine::instance()->getContext().getSubsystem<GraphicsContext>()->getCommandPool(usage))
{
	...
	if (!VK_CHECK(vkAllocateCommandBuffers(Graphics::RenderContext::GetDevice(), &command_buffer_allocate_info, &m_handle)))
	{
...
CommandBuffer::~CommandBuffer()
{
	if (m_handle)
	{
		vkFreeCommandBuffers(Graphics::RenderContext::GetDevice(), *m_command_pool, 1, &m_handle);
	}
}
...
void CommandBuffer::blitImage(const Graphics::Image &src, VkImageUsageFlagBits src_usage, const Graphics::Image &dst, VkImageUsageFlagBits dst_usage, VkFilter filter) const
{
...
void CommandBuffer::generateMipmaps(const Graphics::Image &image, VkImageUsageFlagBits initial_usage, VkFilter filter) const
{
...
void CommandBuffer::transferLayout(const Graphics::Image &image, VkImageUsageFlagBits old_usage, VkImageUsageFlagBits new_usage) const
{
	...
	vkCmdPipelineBarrier(*this, Graphics::Image::UsageToStage(old_usage), Graphics::Image::UsageToStage(new_usage), 0, 0, nullptr, 0, nullptr, 1, &barrier);
}

void CommandBuffer::transferLayout(const std::vector<Graphics::ImageReference> &images, VkImageUsageFlagBits old_usage, VkImageUsageFlagBits new_usage) const
{
	...
	vkCmdPipelineBarrier(*this, Graphics::Image::UsageToStage(old_usage), Graphics::Image::UsageToStage(new_usage), 0, 0, nullptr, 0, nullptr, static_cast<uint32_t>(barriers.size()), barriers.data());
}
