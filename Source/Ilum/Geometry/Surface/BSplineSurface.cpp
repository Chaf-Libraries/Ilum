...
void BSplineSurface::generateVertices(std::vector<Vertex> &vertices, std::vector<uint32_t> &indices, const std::vector<std::vector<glm::vec3>> &control_points, uint32_t sample_x, uint32_t sample_y)
{
...
glm::vec3 BSplineSurface::value(const std::vector<std::vector<glm::vec3>> &control_points, float u, float v)
{
	...
	for (uint32_t i = 0; i < nu; i++)
	{
		...
		{
			result += gen_basis(i, order, u) * gen_basis(j, order, v) * control_points[i][j];
		}
	}
