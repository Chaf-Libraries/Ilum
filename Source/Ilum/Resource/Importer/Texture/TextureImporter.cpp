...
TextureImportInfo TextureImporter::ImportFromBuffer(const std::vector<uint8_t> &raw_data)
{
	...
	if (stbi_is_hdr_from_memory(static_cast<const stbi_uc *>(raw_data.data()), raw_data.size()))
	{
		data = stbi_loadf_from_memory(static_cast<const stbi_uc *>(raw_data.data()), raw_data.size(), &width, &height, &channel, req_channel);
	...
	else if (stbi_is_16_bit_from_memory(static_cast<const stbi_uc *>(raw_data.data()), raw_data.size()))
	{
		data = stbi_load_16_from_memory(static_cast<const stbi_uc *>(raw_data.data()), raw_data.size(), &width, &height, &channel, req_channel);
	...
	{
		data = stbi_load_from_memory(static_cast<const stbi_uc *>(raw_data.data()), raw_data.size(), &width, &height, &channel, req_channel);
