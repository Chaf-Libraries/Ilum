#include "RationalBezierSurface.hpp"
...
void RationalBezierSurface::generateVertices(std::vector<Vertex> &vertices, std::vector<uint32_t> &indices, const std::vector<std::vector<glm::vec3>> &control_points, uint32_t sample_x, uint32_t sample_y)
{
...
glm::vec3 RationalBezierSurface::value(const std::vector<std::vector<glm::vec3>> &control_points, float u, float v)
{
	...
	for (uint32_t i = 0; i < nu; i++)
	{
		...
		{
			float weight = bernstein_basis(nu - 1, i, u) * bernstein_basis(nv - 1, j, v) * weights[i][j];
			denominator += weight;
